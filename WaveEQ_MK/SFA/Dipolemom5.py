...
for ti, tr in zip(ti_list, tr_list):
    ...
    prefactor = ((2*np.pi)/(epsilon+1j*delta_t))**(3/2)  # Prefactor from momentum integration using saddle point, why does this not work?!?!?!? what am i doing wronmg :(

    # Calculate and store the dipole moment
    Dipole.append((1j * prefactor *d_matrix_recol * d_matrix_ion* pulse_field(ti,E0,w,Nc)*np.exp(-1j * Sv)))
